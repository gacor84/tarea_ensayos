...
```{r t_test_despues,echo=FALSE,results='hide'}
# t-test
...
```{r t_test_efecto,echo=FALSE,results='hide'}
# t-test
...
```{r ancova_max,echo=FALSE,results='hide'}
# ajuste
...
```{r ancova_main,echo=FALSE,results='hide'}
# ajuste
...
$\\$
```{r drop1_ancova,echo=FALSE,results='hide'}
step(pol.lm_max)
```
...
```{r ancova_main_medias,echo=FALSE,results='hide'}
# valores medios (log)
pol.lm_main$coefficients[1] # log(polipos) control
pol.lm_main$coefficients[2] # log(polipos) tratamiento
exp(pol.lm_main$coefficients[1]) # polipos control
exp(pol.lm_main$coefficients[2]) # polipos tratamiento
```
...
```{r plot_probs12,echo=FALSE,fig.width=10,fig.height=4}
# plot
...
```{r plot_probs3,echo=FALSE,fig.width=10,fig.height=6}
...
```{r N2.2,echo=FALSE,results='hide'}
bet2.2<-0.10
...
```{r N2.3,echo=FALSE,results='hide'}
bet2.3<-0.05
...
```{r N2.4,echo=FALSE,results='hide'}
p.B2<-0.6
...
```{r datos_linfoma_ANEXO,eval=FALSE}
# datos
dat <- read.table("linfoma.dat",header=T)
efectivo <- as.factor(dat$B3TODEATH>1)
grupo <- as.factor(2-dat$GROUP)
...
```{r sum_datos_grupos_ANEXO,eval=FALSE}
# sumario por grupos
sum.dat.g<-summaryBy(data = datos,formula = efectivo~grupo*efectivo,FUN = length)
colnames(sum.dat.g)<-c("grupo","efectivo","n")
...
```{r hist_datos_grupos_ANEXO,eval=FALSE}
# histogramas por grupos
p1<-ggplot(data = datos,aes(x=grupo,fill=efectivo))+
    geom_histogram(stat="bin",position="dodge")+
    labs(x="",y="número de casos\n",title="")+
...
```{r hist_datos_grupos2_ANEXO,eval=FALSE}
p2<-ggplot(data = sum.datos.g[c(2,4),],aes(x=grupo,y=odds,fill=grupo))+
    geom_histogram(stat="identity")+
    labs(x="",y="odds efectivo / no efectivo\n",title="")+
    scale_fill_discrete(breaks=c("Si.Rad","No.Rad"),name="",
...
```{r sum_dg_table_ANEXO,eval=FALSE}
# sumario por grupos
kable(sum.datos.g,caption = "Efectividad del tratamiento por grupos",digits = 3)
```
...
```{r tabla_cont_ANEXO,eval=FALSE}
datos.table<-with(datos,table(grupo,efectivo))
kable(datos.table,digits=3,caption="Tabla de contingencia (frecuencias observadas)")
```

```{r chi_test_ANEXO,eval=FALSE}
(result<-chisq.test(datos.table,correct = F))
kable(result$expected,caption = "Tabla de contingencia (frecuencias esperadas)",digits=3)
```
...
```{r logit_ANEXO,eval=FALSE}
# summary ajuste
summary(logit)
```

```{r OR_IC_ANEXO,eval=FALSE}
# Odds ratio
...
```{r probs_logit_ANEXO,eval=FALSE}
# Probs logit
logit.coefs<-logit$coefficients # =coef(logit)
...
```{r plot_probs12_ANEXO,eval=FALSE}
# plot KPS medio
p.prob1<-ggplot(data = newdata.logit.fit,aes(x=grupo,y=prob,colour=edad,shape=sexo))+
    geom_point(size=3)+
    geom_line(aes(group=interaction(sexo,edad)))+
...
```{r plot_probs3_ANEXO,eval=FALSE}
